from fastapi import FastAPI
from schema import CustomerData
from typing import List
from model_inference import predict_churn
app=FastAPI(title='Banking Churn Prediction API', description='API for predicting customer churn in banking sector')
@app.get('/',title='Home Page')
def home():
    return {'message': 'Welcome to the home page, Banking Churn Prediction System'}

@app.post('/predict',title='Prediction')
def predict(data:List[CustomerData]):
    result = predict_churn(data)
    return {'status':'success', 'predictions': result}